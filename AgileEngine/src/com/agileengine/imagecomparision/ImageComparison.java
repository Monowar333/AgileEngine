...
//	        		delta = Math.abs((rgb1 / rgb2) * 100);
	        		if(!compareRGB(rgb1, rgb2)){
	        			System.out.println(compareRGB(rgb1, rgb2));
	        		countflag = 0;
	        		if (rectList.size() != 0){
	...
	private void rectanglesMarge(){
		...
		while(countflag != 0){
		 ...
		 for(Rectangles rectRes: rectList){
			 ...
			 for(Rectangles rectIn: rectList){
				 if((rectIn.isFlag()) &&
					(!rectIn.equals(rectRes))&&
					((rectIn.getXmax() <= rectRes.getXmax() &&
				      rectIn.getXmax() >= rectRes.getXmin() &&
				   	  rectIn.getYmax() <= rectRes.getYmax() &&
				   	  rectIn.getYmax() >= rectRes.getYmin()) ||
					 (rectIn.getXmin() <= rectRes.getXmax() &&
					  rectIn.getXmin() >= rectRes.getXmin() &&
